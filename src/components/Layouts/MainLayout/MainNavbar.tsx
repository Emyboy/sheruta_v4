            ...
            <header className='bg-dark h-16 flex items-center justify-center fixed left-0 right-0 z-50 '>
                <_Container className="flex  justify-between px-4 w-full">
                    ...
                    <nav className='hidden lg:visible lg:flex items-center gap-2'>
                        <EachNav Icon={(p: IconBaseProps) => <HiOutlineHome {...p} />} active={activePage === 'home'} />
                        <EachNav Icon={(p: IconBaseProps) => <HiPlus {...p} />} active={activePage === 'update'} />
                        <EachNav Icon={(p: IconBaseProps) => <HiOutlineBell {...p} />} active={activePage === 'activities'} link='/activities' />
                        <EachNav Icon={(p: IconBaseProps) => <HiMagnifyingGlass {...p} />} active={activePage === 'search'} />
                    </nav>

                    <nav className='flex items-center gap-2'>
                        <Link href='/' className='hidden sm:visible md:flex outline-1 outline px-5 py-2 rounded-lg text-theme_light text-md hover:bg-theme hover:text-white hover:outline-none'>
                            Post Request
                        </Link>
