        ...
        <>
            ...
            <div className="bg-black- h-80 lg:h-screen w-screen flex justify-center items-center lg:w-2/3 lg:px-5 flex-col z-40" >
                <div className=' p-2 flex flex-col items-center justify-center h-64 lg:h-5/6' >
                    <img src={currentPreview.url} className='h-full lg:rounded-lg shadow-lg rounded-md' />
                </div>
                <div className='p-2 w-full flex gap-3 justify-center- h-16 lg:h-1/6 overflow-x-auto bg-img'>
                    ...
                    {images.map(src => {
                        return <div className={classNames({ "outline outline-4 lg:outline-3 text-green-400": currentPreview.url === src.url }, ' cursor-pointer bg-img h-full rounded-md lg:w-52')} style={{ backgroundImage: `url(${src.url})`, minWidth: '70px' }} onClick={() => setCurrentPreview(src)} />
                    })}
                </div>
            </div>
        </>
